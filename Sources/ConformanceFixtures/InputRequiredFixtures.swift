...
enum RequestState {
    /// A per-process key, held as bytes rather than as a `SymmetricKey`.
    ///
    /// The token only has to outlive the exchange, and a key that does not outlive the process
    /// cannot leak into one. What it is *stored* as is a portability question, not a
    /// cryptographic one: on Apple platforms `Crypto` re-exports CryptoKit, whose `SymmetricKey`
    /// is `Sendable`, while on Linux it is swift-crypto's own type, which is not. A
    /// `static let SymmetricKey` therefore compiles here and fails to build on Linux under
    /// strict concurrency — a platform this package claims to support and does not test.
    ///
    /// `[UInt8]` is `Sendable` on both, so the key is stored as bytes and wrapped at each use.
    /// That is a cheap wrap and it needs no `@unchecked` conformance to justify.
    /// Generated by `SymmetricKey`'s own CSPRNG and then copied out, rather than by
    /// `UInt8.random`. Injectable seeding is right for a stochastic test and wrong for a signing
    /// key — a key a caller can make deterministic is a key an attacker can predict — so the
    /// randomness stays where the cryptography library put it.
    private static let keyMaterial: [UInt8] = {
        var bytes = [UInt8](repeating: 0, count: 32)
        // Copied out scalar by scalar rather than with `[UInt8](raw)`. Handing the block's
        // pointer to an initializer is an escape as far as the pointer rules are concerned, and
        // correctly so — an initializer may keep what it is given. Copying bytes keeps the
        // pointer inside the block, which is where it has to stay.
        SymmetricKey(size: .bits256).withUnsafeBytes { raw in
            for index in 0..<min(bytes.count, raw.count) { bytes[index] = raw[index] }
        }
        return bytes
    }()

    /// The signing key, rebuilt from ``keyMaterial`` at each use.
    private static var key: SymmetricKey { SymmetricKey(data: keyMaterial) }
